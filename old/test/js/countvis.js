...
//    this.addSlider();
    ...
    this.wrangleData(function(d) {return d.species == "Oyster" && d.state == "Maryland";});
...
CountVis.prototype.updateVis = function(){
 ...
 //   this.xScale.domain(d3.range(that.displayData.length));//
    ...
    this.yScale.domain([0, d3.max(that.displayData, function(d) { return d.pounds; })]);
...
//    this.xAxis.tickFormat(function(d) { return d.year;});
    ...
    bar
        .attr("class", "bar")
      .style("fill", function(d) { if (d.state == "Maryland") {return "blue";} else {return "red";}})
        .attr("transform", function(d,i) {return "translate(" + that.xScale(d.year) + " , " + that.yScale(d.pounds) + ")"; });
    ...
    bar.selectAll("rect")
        .attr("x", 0)
        .attr("y", 0)
        .attr("height", function(d) { return that.height - that.yScale(d.pounds);})
...
CountVis.prototype.onSelectionChange= function (species){
    console.log(species);
    this.wrangleData(function(d) {return d.species == species && d.state == "Maryland";});
    this.updateVis();
}
